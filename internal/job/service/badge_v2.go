...
func init() {
	contractAbi, err := abi.JSON(strings.NewReader(ABIV2.BadgeV2MetaData.ABI))
	if err != nil {
		panic(err)
	}
...
func (s *Service) handleClaimedV2(client *ethclient.Client, hash string, vLog *types.Log) (err error) {
	var claimed ABIV2.BadgeV2Claimed
	if err = questMinterAbiV2.UnpackIntoInterface(&claimed, "Claimed", vLog.Data); err != nil || len(vLog.Topics) == 0 {
