...
func (s *Service) GolangTryRun(runReq request.TryRunReq) (tryRunRes response.TryRunRes, err error) {
	...
	codeSnippetRaw := gjson.Get(string(runReq.Quest.QuestData), fmt.Sprintf("questions.%d.code_snippets.#(lang=Golang).code", runReq.QuestIndex)).String()
	...
	// 请求体
	javaScriptRun, err := s.GolangRun(runReq.Code, codeSnippetRaw, inputs)
	if err != nil {
...
func (s *Service) GolangTryTestRun(runReq request.TryTestRunReq) (tryRunRes response.TryRunRes, err error) {
	// 请求体
	javaScriptRun, err := s.GolangRun(runReq.ExampleCode, runReq.CodeSnippet, runReq.ExampleInput)
	if err != nil {
		log.Errorv("JavaScriptRun error", zap.Error(err))
		return tryRunRes, err
	}
