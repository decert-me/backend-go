...
func (s *Service) JavaScriptTryRun(runReq request.TryRunReq) (tryRunRes response.TryRunRes, err error) {
	...
	codeSnippetRaw := gjson.Get(string(runReq.Quest.QuestData), fmt.Sprintf("questions.%d.code_snippets.#(lang=JavaScript).code", runReq.QuestIndex)).String()
	...
	// 请求体
	javaScriptRun, err := s.JavaScriptRun(runReq.Code, codeSnippetRaw, inputs)
	if err != nil {
...
func (s *Service) JavaScriptTryTestRun(runReq request.TryTestRunReq) (tryRunRes response.TryRunRes, err error) {
	// 请求体
	javaScriptRun, err := s.JavaScriptRun(runReq.ExampleCode, runReq.CodeSnippet, runReq.ExampleInput)
	if err != nil {
		log.Errorv("JavaScriptRun error", zap.Error(err))
		return tryRunRes, err
	}
