...
func (s *Service) TypeScriptTryRun(runReq request.TryRunReq) (tryRunRes response.TryRunRes, err error) {
	...
	codeSnippetRaw := gjson.Get(string(runReq.Quest.QuestData), fmt.Sprintf("questions.%d.code_snippets.#(lang=TypeScript).code", runReq.QuestIndex)).String()
	...
	// 请求体
	javaScriptRun, err := s.TypeScriptRun(runReq.Code, codeSnippetRaw, inputs)
	if err != nil {
...
func (s *Service) TypeScriptTryTestRun(runReq request.TryTestRunReq) (tryRunRes response.TryRunRes, err error) {
	// 请求体
	javaScriptRun, err := s.TypeScriptRun(runReq.ExampleCode, runReq.CodeSnippet, runReq.ExampleInput)
	if err != nil {
		log.Errorv("TypeScriptRun error", zap.Error(err))
		return tryRunRes, err
	}
	// 发送请求到判题沙箱
	client := req.C().SetTimeout(30 * time.Second)
